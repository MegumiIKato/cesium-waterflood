def extract_node_depth_data(rpt_file_path):
    """
    从rpt文件中提取Node、AverageDepthMeters和Total Flood Volume数据
    
    参数:
        rpt_file_path: rpt文件路径
        
    返回:
        提取的Node和对应的Average Depth数据列表，以及洪水体积数据列表
    """
    node_depth_data = []
    node_flood_data = []
                ...
                if len(parts) >= 6:
                    ...
                    try:
                        ...
                        if flood_volume > 0:  # 忽略值为0.000的数据
                            ...
                            # print(f"找到洪水体积数据: {node_id} - {flood_volume}")  # 调试信息
